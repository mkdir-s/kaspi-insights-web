...
function buildExportReport(statement: StatementData, analytics: Analytics) {
  ...
  const incomingTransferMonthRows = analytics.incomingTransfersByMonth.map((item) => `<tr><td>${escapeHtml(item.label)}</td><td>${item.count}</td><td class="in">${formatMoney(item.amount)}</td></tr>`).join("");
  ...
  <section><h2>Полученные переводы по месяцам</h2><table><thead><tr><th>Месяц</th><th>Количество переводов</th><th>Полученная сумма</th></tr></thead><tbody>${incomingTransferMonthRows}</tbody></table></section>
...
function TransferTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number; payload: { amount: number } }>; label?: string }) {
  if (!active || !payload?.length) return null;
  const item = payload[0];
  return <div className="chart-tooltip transfer-tooltip"><b>{label}</b><div><i style={{ background: "#22a06b" }} />Получено<strong>{item.value} переводов</strong></div><div><i style={{ background: "#7b61ff" }} />Сумма<strong>{formatMoney(item.payload.amount)}</strong></div></div>;
}
...
function EmptyState({ onFile }: { onFile: (file: File) => void }) {
  ...
  return <main className="landing">
    ...
    <section className="statement-guide">
      <div className="guide-heading"><div className="eyebrow"><FileText size={15} /> Подготовка файла</div><h2>Как получить выписку в Kaspi Gold</h2><p>Сформируйте полный период — так статистика за текущий год будет полезнее и точнее.</p></div>
      <div className="guide-steps">
        <div><span>01</span><i><WalletCards /></i><b>Откройте Kaspi Gold</b><p>В приложении Kaspi.kz выберите свою карту Gold.</p></div>
        <div><span>02</span><i><ReceiptText /></i><b>Перейдите в «Выписка»</b><p>В меню карты откройте раздел с выпиской.</p></div>
        <div><span>03</span><i><CalendarDays /></i><b>Укажите период</b><p>С 1 января {currentYear} года по сегодняшний день.</p></div>
        <div><span>04</span><i><FileDown /></i><b>Выгрузите в PDF</b><p>Скачайте PDF-файл и загрузите его в область выше.</p></div>
      </div>
    </section>
...
function Dashboard({ statement, onReset }: { statement: StatementData; onReset: () => void }) {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard" id="top">
      ...
      <section className="panel incoming-monthly-panel" id="transfers"><SectionHeading eyebrow="Входящие переводы по месяцам" title="Сколько переводов вы получили" note={`${analytics.incomingTransfers.count} переводов за весь период`} /><div className="transfer-month-layout"><div className="chart-medium"><ResponsiveContainer width="100%" height="100%"><BarChart data={analytics.incomingTransfersByMonth}><CartesianGrid vertical={false} stroke="#edf0ee" /><XAxis dataKey="label" axisLine={false} tickLine={false} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} width={34} /><Tooltip content={<TransferTooltip />} /><Bar name="Получено переводов" dataKey="count" fill="#22a06b" radius={[7, 7, 0, 0]} /></BarChart></ResponsiveContainer></div><div className="month-count-list">{analytics.incomingTransfersByMonth.map((item) => <div key={item.month}><span>{item.label}<small>{formatMoney(item.amount)}</small></span><strong>{item.count}</strong><b>переводов</b></div>)}</div></div></section>
